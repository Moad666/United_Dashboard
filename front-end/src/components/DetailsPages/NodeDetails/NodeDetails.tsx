...
import { getLinkFromNode } from '@/utils/node/getLinksFromNode'
...
export default function NodeDetails({ node }: { node: NodeProps }) {
  console.log({ node })
...
function ShowAppInfo({ data }: { data: NodeProps }) {
...
interface ShowDataProps {
  name: string | keyof NodeProps
  value: NodeProps[keyof NodeProps]
}
...
function LinkSection({ app: node }: { app: NodeProps }) {
  // get links from the application
  const linkGroups = getLinkFromNode(node).filter((link) => link)
  return (
    <div>
      <div>
